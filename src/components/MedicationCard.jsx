...
import { formatDate, sortDelayedPatients } from '../utils/helpers'
import SyncBadge from './SyncBadge'

// Each medication gets a consistent color pair derived from its name
const PALETTES = [
...
export default function MedicationCard({ medication }) {
  ...
  return (
    <div
      className="card cursor-pointer group overflow-hidden hover:-translate-y-1 hover:shadow-xl transition-all duration-200"
    ...
    >
      ...
      <div className={`bg-gradient-to-br ${palette.bg} px-5 pt-5 pb-4 relative overflow-hidden`}>
        ...
        <div className="relative flex items-start justify-between gap-2">
          ...
          <div className="relative shrink-0" ref={menuRef} onClick={e => e.stopPropagation()}>
            <button
              className="p-1.5 rounded-lg text-white/60 hover:text-white hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
              onClick={() => setMenuOpen(o => !o)}
            >
              <MoreVertical size={15} />
            </button>
            {menuOpen && (
              <div className="absolute right-0 top-8 w-40 bg-white rounded-xl shadow-xl border border-slate-100 py-1 z-10 animate-slide-up">
                <button
      ...
      <div className="p-5">
        ...
        {total > 0 && (
          <div className="mb-4">
            ...
            <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
              <div
                className={`h-full bg-gradient-to-r ${palette.bar} rounded-full transition-all duration-500`}
                style={{ width: `${scheduledRatio * 100}%` }}
              />
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between text-xs text-slate-400 pt-3 border-t border-slate-50">
          ...
          <span className="flex items-center gap-1 text-indigo-400 font-medium group-hover:text-indigo-600 transition-colors">
